...
class RandomDSDChemistry():
    ...
    def create_domain_lookup(self):
        ...
        switching_domains = list(SWITCHING_DOMAINS[:self.input_params['s']])

        self.domain_lookup = {'U': {}, 'L': {}}
        for u in self.U:
            switching_domain = str(np.random.choice(switching_domains))
    ...
    def create_species_double(self):
        ...
        for u in self.U:
            for l in self.L:
                if self.domain_lookup['U'][u][2] == self.domain_lookup['L'][l][2][:-1]: # check if switching domains are the same type for upper and lower strands (exclude the *)
                    if self.domain_lookup['U'][u][3] == self.domain_lookup['L'][l][3][:-1]: # check if identity domains are the same type for upper and lower strands (exclude the *)
                        self.F.append(u + l)
                    else:
                        self.PS.append(u + l)
                else:
    ...
    def create_reaction_displacement(self):
        ...
        for u in self.U: # for each upper strand
            for ps in self.PS: # for each strong partial double strand
                ...
                if u not in ps: # if single strand is not part of double strand
                    if (self.domain_lookup['U'][u][2] == self.domain_lookup['L'][dl][2][:-1]) and (self.domain_lookup['U'][u][3] == self.domain_lookup['L'][dl][3][:-1]): # if the switching and identity domain of the upper strand matches that of the lower part of the double strand
                        prod = u + dl
            ...
            for pw in self.PW: # for each weak partial double strand
                ...
                if u not in pw: # if single strand is not part of double strand
                    if self.domain_lookup['U'][u][2] == self.domain_lookup['L'][dl][2][:-1]: # if the switching domain of the upper strand matches that of the lower part of the double strand
                        prod = u + dl
        ...
        for l in self.L: # for each lower strand
            for ps in self.PS: # for each strong partial double strand
                du, dl = self.decode_double(ps)
                if u not in ps: # if single strand is not part of double strand
                    if (self.domain_lookup['U'][du][2] == self.domain_lookup['L'][l][2][:-1]) and (self.domain_lookup['U'][u][3] == self.domain_lookup['L'][dl][3][:-1]): # if the switching and identity domain of the upper strand matches that of the lower part of the double strand
                        prod = du + l
            ...
            for pw in self.PW: # for each weak partial double strand
                du, dl = self.decode_double(pw)
                if u not in pw: # if single strand is not part of double strand
                    if self.domain_lookup['U'][du][2] == self.domain_lookup['L'][l][2][:-1]: # if the switching domain of the lower strand matches that of the upper part of the double strand
                        prod = du + l
...
if __name__ == '__main__':
    ...
    randDSDChem = RandomDSDChemistry(input_params, time_params)
