...
df_NRMSE_mean = df_NRMSE.pivot(index='Base Influx Rate (species/sec)',columns='tau (sec)',values='NRMSE_means')
df_NRMSE_stds = df_NRMSE.pivot(index='Base Influx Rate (species/sec)',columns='tau (sec)',values='NRMSE_stds')
ax_NRMSE = df_NRMSE_mean.plot(kind='bar', yerr=df_NRMSE_stds.values, capsize=3, grid=True, rot=0)
ax_NRMSE.grid(linestyle=':')
ax_NRMSE.legend(loc='upper left', bbox_to_anchor=(0.11, 1), handlelength=1.0, framealpha=0.5, title=r'$\tau$ (s)', fontsize='medium')
