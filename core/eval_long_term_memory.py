...
for input_param in input_params:
    for time_param in time_params:
        ...
        for i in range(num_exp):
            ...
            NRMSE, fitness = long_term_memory(input_params=input_param, time_params=time_param, num_epoch=num_epoch)
...
df_NRMSE.to_csv('dat/LT_NRMSE_{}ep_{}ex.csv'.format(num_epoch, num_exp), index=False)

df_fitness = pd.DataFrame({'input range (species/sec)': base_influx_list,                                                                                  
                           'tau (sec)': hold_time_list,                                                                                  
                           'fitness_means': fitness_means,
                           'fitness_stds': fitness_stds})
df_fitness.to_csv('dat/LT_fitness_{}ep_{}ex.csv'.format(num_epoch, num_exp), index=False)
